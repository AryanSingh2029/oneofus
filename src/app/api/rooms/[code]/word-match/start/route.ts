...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    if (room.game_id !== "word-match") {
      return NextResponse.json(
        { error: "Separate-phone rounds are only wired for Word Match right now." },
        { status: 400 },
      );
    }
    ...
    if (!players || players.length < 3) {
      return NextResponse.json(
        { error: "Word Match needs at least 3 players." },
        { status: 400 },
      );
    }

    const pair = wordMatchPairs[
      Math.floor(Math.random() * wordMatchPairs.length)
    ] as WordPair;
